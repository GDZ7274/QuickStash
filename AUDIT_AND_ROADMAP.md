...
版本号已统一为 `1.1.0 (4)`。重复复制的文字、HTTP(S) 链接和图片会保留原记录 ID、固定状态和图片 canonical 路径，并按最新复制时间回到列表前方；bootstrap 合并会把 incoming ID 映射到 canonical ID，使期间发生的删除、固定、复制确认和迟到图片复制继续命中保留项。损坏的旧图片路径会在后台重新解码核验失败后由新 PNG 接管；替换和冗余图片清理均先持久化 intent，崩溃后可继续恢复。截图使用 ScreenCaptureKit，不使用 `CGWindowListCreateImage`，没有外部依赖、辅助功能权限请求或虚构的 plist 截图权限键。
...
- 导入、删除和孤儿恢复使用持久化 manifest。新版删除 manifest 保存源是否存在及其设备号、inode；恢复、显式提交和延迟确认都会核对文件身份，同一路径被新文件复用时只废弃旧 intent，不移动或删除新文件。旧格式 manifest 保持可解码。应用重启后可恢复已复制但尚未进入 metadata 的文件，并清理 partial、orphan 和中断删除状态。
- `items.json` 采用带 schema version 的原子写入，支持旧数组迁移、损坏备份、未来版本只读保护、revision 去旧和退出前异步 flush。
- 剪贴板首次使用要求明确授权，“启用实时记录”为主操作；监听可关闭，以 `0.25` 秒间隔记录文字、HTTP(S) 链接及系统可解码图片，过滤 concealed/transient 类型，但不会误伤带 `AutoGeneratedType` 标记的合法图片，并有限额和保留数量/期限。PNG、TIFF、JPEG、HEIC/HEIF、WebP 等来源统一转为真实 PNG 落盘。文字和 URL 按 type、origin 与精确内容去重；图片按固定 sRGB RGBA8 像素指纹去重，复用前重新解码并核对实际文件指纹。纯文字和 URL 一次稳定读取即提交；只有明确声明但尚未就绪的图片表示才保留后备文字并有限重试。后台读取绑定稳定 `changeCount`，最多保留两条物理 provider lane；软超时后的迟到结果只能提交原计数，内部写入只抑制自己的精确计数。
- bootstrap 重放通过 incoming-to-canonical ID alias 把删除、固定、复制计数、清理状态和迟到图片复制归并到最终记录。跨 backing 的迟到图片复制只有在存在显式 alias 且像素指纹一致时才可命中，避免把不相关文件当成同一记录。
- 重复图片的新 backing 在丢弃前先写入 deletion manifest；移动失败时保留 intent，由启动恢复重试。损坏 canonical backing 的替换按“写 replacement intent -> 切换并刷入 metadata -> 隔离旧 backing -> 确认 manifest”执行，避免 metadata 与文件移动之间的崩溃窗口。
- 启动恢复会按像素指纹迁移旧版本遗留的同像素记录：保留 canonical ID、合并固定状态并采用最新时间。若持久化 canonical 已损坏但存在有效同指纹 orphan，则保留 canonical 身份并接管有效 backing；无引用且无法解码的图片 orphan 进入隔离区，不会恢复到历史。
- 正常退出按 8 秒全局总预算依次处理截图输出、剪贴板工作和 metadata flush；排空完成或预算到期都只调用一次 `reply(toApplicationShouldTerminate:)`。超时不会无限等待底层 I/O，可恢复的删除或替换由 manifest 在下次启动继续。
...
- 生产退出路径为 `applicationShouldTerminate -> TerminationDeadlineCoordinator`，异步执行截图、剪贴板和 metadata 排空；协调器使用 8 秒总预算，完成与超时竞争通过同一单次完成门禁，保证只回复 macOS 一次。
...
- `ClipboardMonitor.drainPayloadReads` 有且只有一次带截止时间的 `DispatchGroup.wait(timeout:)`，它被派发到全局 utility queue 后再通过 continuation 返回；`MainActor` 不执行该等待，也不存在无界 semaphore/group wait。`FileImportProgressEmitter.finish()` 最多等待一次节流窗口，但它只在 file-I/O queue 上运行。
- 生产源码没有主线程 `DispatchQueue.sync` 或 `NSFileCoordinator`。任何新增同步等待都必须证明不在 `MainActor`、有严格上限且不会形成 queue 反向依赖。
...
所有验证均使用 `/tmp` 或隔离的 `CFFIXED_USER_HOME`，没有读写真实 `~/Library/Application Support/QuickStash`。本轮在恢复与退出逻辑停止变更后重新从零执行最终验证，没有沿用早期构建的累计次数或二进制结果。

最终门禁全部通过：`verify_without_xcode.sh` 单次及连续 20 次、干净 Debug/Release、显式 strict concurrency、warnings-as-errors、Xcode Analyze、Hosted XCTest 33/33、适用的 Thread Sanitizer 32/32、真实 ScreenCaptureKit 100/100 轮压力、DMG 校验，以及隔离 HOME 的可见 Release App 启动。各项结果和仍需人工验证的环境边界统一记录在 `docs/VALIDATION.md`。
